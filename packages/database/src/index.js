...
const configuredDatabasePath = String(
  process.env.LIGA_ARENA_DATABASE_PATH || "",
).trim();

const databasePath = configuredDatabasePath
  ? path.resolve(configuredDatabasePath)
  : path.join(dataDirectory, "liga-arena.db");
...
const hasLogoColumn = teamColumns.some(
...
`);
/*
 * Existing registration modal profile fields.
 * Idempotent migration for development and production.
 */
...
/*
 * LIGA_ARENA_LEAGUE_ASSIGNMENT
 * Admin tasdiqlangan jamoani liga darajasiga biriktiradi.
 */
...
/*
 * LIGA_ARENA_SCHEDULE_EVENTS
 * PUBG va MLBB tadbirlari uchun umumiy jadval.
 */
...
`);
/*
 * LIGA_ARENA_LEAGUE_STANDINGS
 * PUBG va MLBB liga turnir jadvallari.
 */
...
`);
/*
 * LIGA_ARENA_SMART_COMPETITION_V1
 *
 * - Avtomatik texnik raqam
 * - 4 kartalik turlar
...
const technicalNumberGroups = db
  .prepare(`
    SELECT DISTINCT
      season,
      game
...
for (
  const group of technicalNumberGroups
) {
  ...
  db.prepare(`
    ...
    DO UPDATE SET
      last_number = MAX(
        team_number_sequences.last_number,
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS
    idx_competition_rounds_division
  ...
  CREATE INDEX IF NOT EXISTS
    idx_competition_round_teams_team
...
`);
/*
 * LIGA_ARENA_COMPETITION_FINALIZATION_V1
 *
 * Saralashni yakunlash va keyingi mavsum
 * liga joylashuvlarini saqlash.
 */
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS
    idx_competition_finalizations_division
  ...
  CREATE INDEX IF NOT EXISTS
    idx_competition_final_results_rank
...
`);
/*
 * LIGA_ARENA_SEASON_ROLLOVER_V1
 *
 * Yakunlangan mavsumdan keyingi mavsumga
 * jamoalarni xavfsiz o‘tkazish.
 */
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS
    idx_season_rollover_teams_movement
...
`);
/*
 * LIGA_ARENA_ACTIVE_SEASON_V1
 *
 * Platformadagi joriy aktiv mavsum.
 */
...
`);
/*
 * LIGA_ARENA_MLBB_FINALIZATION_V1
 */
...
`);
/*
 * LIGA_ARENA_ROUND_TELEGRAM_NOTIFICATIONS_V1
 *
 * PUBG / MLBB tur bildirishnomalari.
...
`);
/*
 * LIGA_ARENA_PUBG_FINALIZATION_V1
 *
 * PUBG saralash mavsumining rasmiy
 * va o‘zgarmaydigan yakuniy natijasi.
 */
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS
    idx_competition_finalizations_lookup
